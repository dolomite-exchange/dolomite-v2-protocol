...
const defaultMaxSupplyWei = INTEGERS.ZERO;
const defaultMaxBorrowWei = INTEGERS.ZERO;
const defaultEarningsRateOverride = INTEGERS.ZERO;
...
describe('LiquidatorProxyV4WithGenericTrader', () => {
  before(async () => {
    ...
    liquidAccount = { owner: liquidOwner, number: liquidNumber.toFixed() };
    ...
    it('should fail when not called by a global operator', async () => {
      await expectThrow(
        dolomiteMargin.contracts.callContractFunction(
          dolomiteMargin.contracts.eventEmitterRegistry.methods.emitZapExecuted(
            solidOwner,
            solidNumber.toFixed(),
            simpleMarketIdPath.map(m => m.toFixed()),
            [],
          ),
          { from: solidOwner },
        ),
      ...
      await expectThrow(
        dolomiteMargin.contracts.callContractFunction(
          dolomiteMargin.contracts.eventEmitterRegistry.methods.emitBorrowPositionOpen(
            solidOwner,
            solidNumber.toFixed(),
          ),
          { from: solidOwner },
  ...
  describe('#liquidate', () => {
    ...
    describe('Success cases', () => {
      ...
      it('should succeed when there is a risk override', async () => {
        await Promise.all([
          ...
          dolomiteMargin.testing.setAccountBalance(liquidOwner, liquidNumber, market5, INTEGERS.ZERO),
          dolomiteMargin.testing.setAccountBalance(liquidOwner, liquidNumber, market6, INTEGERS.ZERO),
          dolomiteMargin.testing.accountRiskOverrideSetter.setAccountRiskOverride(liquidAccount, new BigNumber('0.1'), new BigNumber('0.03')),
    ...
    describe('Failure cases', () => {
      ...
      it('should fail when owed market eq held market', async () => {
        await expectThrow(
          dolomiteMargin.liquidatorProxyV4WithGenericTrader.liquidate(
            solidOwner,
            solidNumber,
          ...
          'LiquidatorProxyBase: Owed market equals held market',
        );
      });

      it('should fail when attempting to liquidate assets not held by owner', async () => {
        await Promise.all([
          dolomiteMargin.testing.setAccountBalance(solidOwner, solidNumber, market1, INTEGERS.ZERO),
          dolomiteMargin.testing.setAccountBalance(liquidOwner, liquidNumber, market1, INTEGERS.ZERO),
        ]);
